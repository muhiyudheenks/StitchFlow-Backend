...
import * as inventoryService from '../../inventory/services/adminInventory.service';
...
export async function getAssignedBatches(managerId?: string) {
    let filter: any = {};
    if (managerId) {
        const managerObjectId = mongoose.Types.ObjectId.isValid(managerId)
            ? new mongoose.Types.ObjectId(managerId)
...
export async function getPendingTasks(managerId?: string) {
    return await Task.find({ status: { $in: ['Pending', 'pending', 'in_progress', 'In Progress'] } }).sort({ createdAt: -1 });
}

export async function getTodayAttendance(managerId?: string) {
