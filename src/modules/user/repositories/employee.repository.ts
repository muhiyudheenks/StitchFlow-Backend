...
export async function createEmployeeRepo(dto: CreateEmployeeDto): Promise<IUser> {
...
export async function findEmployeeById(id: string): Promise<IUser | null> {
    return await User.findOne({ _id: id, role: 'employee' })
        .populate('managerId', 'fullName email department');
}

export async function findEmployeeByEmail(email: string): Promise<IUser | null> {
...
export async function updateEmployeeRepo(id: string, dto: UpdateEmployeeDto): Promise<IUser | null> {
...
export async function deleteEmployeeRepo(id: string): Promise<IUser | null> {
    return await User.findOneAndDelete({ _id: id, role: 'employee' });
}

export async function countEmployees(filter: any = {}): Promise<number> {
    return await User.countDocuments({ role: 'employee', ...filter });
}

export const employeeRepository = {
    create: createEmployeeRepo,
    findById: findEmployeeById,
    findByEmail: findEmployeeByEmail,
    findAll: findAllEmployees,
    update: updateEmployeeRepo,
    delete: deleteEmployeeRepo,
