...
export async function getAllTasks(filterQuery?: any) {
    let query: any = {};
...
export async function getTasksByBatch(batchId: string) {
    return await getAllTasks({ batchId });
}
...
export async function createTask(data: any, createdBy: string) {
    ...
    if (!assignedEmployeeId) {
        return await dispatchBatchTask(data, createdBy);
    }
...
export async function dispatchBatchTask(data: any, createdBy: string) {
    const batchId = data.batchId;
    if (!batchId) {
...
export async function verifyTask(taskId: string, status: 'Verified' | 'Completed' | 'Rejected', managerId: string) {
    const task = await Task.findById(taskId);
    if (!task) {
        throw new Error('Task not found');
    }
...
export async function addTaskToInventory(taskId: string, managerId: string) {
