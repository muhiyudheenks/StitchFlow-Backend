...
export async function createInventoryRepo(
...
export async function updateInventoryRepo(
    id: string,
...
export async function deleteInventoryRepo(
    id: string
): Promise<IInventory | null> {
    return await Inventory.findByIdAndDelete(id);
}
...
export async function adjustInventoryStock(
    ...
    if (newQty < 0) {
        throw new Error(
            `Insufficient stock. Current stock: ${item.stockQuantity}, requested reduction: ${Math.abs(delta)}`
        );
    }
...
export async function findLowStock(): Promise<IInventory[]> {
    return await Inventory.find({
        $expr: {
