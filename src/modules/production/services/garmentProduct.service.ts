...
export class GarmentProductService {
    ...
    async createGarmentProduct(data: any, createdBy: string) {
        ...
        if (!productCode) {
            productCode = await this.generateNextProductCode();
        } else {
    ...
    async getGarmentProducts(queryParams: { search?: string; category?: string; status?: string; page?: number; limit?: number }) {
    ...
    async getActiveGarmentProducts() {
    ...
    async getGarmentProductById(id: string) {
    ...
    async updateGarmentProduct(id: string, updateData: any) {
    ...
    async toggleGarmentProductStatus(id: string) {
        const product = await GarmentProduct.findById(id);
        if (!product) {
            throw AppError.notFound('Garment product not found');
        }
    ...
    async deleteGarmentProduct(id: string) {
        const product = await GarmentProduct.findById(id);
        if (!product) {
            throw AppError.notFound('Garment product not found');
        }

        // Check if used in ProductionBatch or Task
