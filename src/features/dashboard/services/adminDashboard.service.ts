import { EmployeeRepository } from '../../user/repositories/employee.repository';
import { ManagerRepository } from '../../user/repositories/manager.repository';
import { ProductionRepository } from '../../production/repositories/production.repository';
...
export class AdminDashboardService {
    ...
    async getOverviewCards(): Promise<OverviewCardsData> {
        const [
            totalEmployees,
            totalManagers,
            ...
            this.attendanceRepo.countTodayPresent(),
    ...
    async getInventoryStatus() {
        const summary = await this.inventoryRepo.aggregateSummary();
        const lowStockList = await this.inventoryRepo.findLowStock();
    ...
    async getAnalyticsSummary(): Promise<AnalyticsSummary> {
        const [
            ...
            this.employeeRepo.count(),
            this.employeeRepo.count({ status: 'active' }),
            this.employeeRepo.count({ status: 'inactive' }),
            this.employeeRepo.count({ status: 'on_leave' }),
            this.managerRepo.count(),
            this.attendanceRepo.getTodayStats(),
            this.productionRepo.aggregateStats(),
            this.productionRepo.count({ status: 'pending' }),
            this.productionRepo.count({ status: 'in_progress' }),
            this.productionRepo.count({ status: 'completed' }),
            this.productionRepo.count({ status: 'delayed' }),
            this.inventoryRepo.aggregateSummary(),
        ]);
